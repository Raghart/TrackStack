...
const DataSlider = ({ params }:{ params: SlidersDataTypes }) => {
    ...
    return(
        <Flex direction="column" p={3} bg="gray.800" borderRadius="full" border="1px solid" 
        borderColor="gray.600" boxShadow="lg" transition="all 0.2s ease-in-out" align="center"
        _hover={{ transform: "scale(1.02)", borderColor: "blue.400", boxShadow: BLUE_SHADOW_MODAL }} 
        w="full" h="120px" maxW={LARA_OPT_SIZES} justify="center">

            <Heading fontSize="20px" mb={2} color="gray.100" textAlign="center" userSelect="none" 
                fontWeight="700" fontFamily="'Barlow Condensed', sans-serif" letterSpacing="wider">
                {params.title}
            </Heading>

            <Slider.Root min={params.min} value={[sliderValue]} max={params.max} colorScheme="blue" 
                cursor="pointer" transition="all 0.3s ease" step={params.step} 
                onValueChange={(e) => setSliderValue(e.value[0])} size="lg" w={SLIDER_SIZES}
                onValueChangeEnd={(e) => dispatch(params.setValue(e.value[0]))}>
                <Slider.Control>
                    ...
                    <Slider.Marks marks={params.max === 1 ? [0.30, 0.70] : [70, 170]} />
                </Slider.Control>
            </Slider.Root>
